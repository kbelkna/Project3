...
Another discrete analysis performed here is the two-way contingency table related to the discretization of the response variable if we divided `shares` into two categories. The function `cut()` was used for the end. In this case, we count the frequency of the number of publications in the weekend versus working days with the two levels of response variable. These levels represent smaller number of shares (on the left) and larger number of shares (on the right). The table below shows this counting. In the table below, 0 (zero) represents working days and 1 (one) represents weekends.
...
The following box plot shows the number of shares for this data channel.  

```{r boxsharesGraph}

boxSharesGraph <- ggplot(statsData, aes(x = dataChannel, y = shares))
boxSharesGraph + geom_boxplot() + 
  ggtitle("Number of Shares for Data Channel") +
